...
def train(train_phases, model, minibatch, minibatch_eval, model_eval, eval_val_every):
    ...
    for ip, phase in enumerate(train_phases):
        ...
        time_sample += time.time() - sample_start
        printf("Total sampling time: {:6.2f} sec".format(time_sample), style='red')
        for e in range(epoch_ph_start, int(phase['end'])):
            ...
            printf("training time: {:6.2f} sec".format(time_train_ep), style='bold')
    ...
    printf("Total sampling time: {:6.2f} sec".format(time_sample), style='red')
    printf("Total validing time: {:6.2f} sec".format(time_eval), style='red')
    printf("Total time: {:6.2f} sec".format(time.time()-train_s), style='red')
    return f1mic_test
