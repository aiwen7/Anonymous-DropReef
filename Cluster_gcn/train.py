...
flags = tf.flags
FLAGS = flags.FLAGS
flags.DEFINE_boolean('use_DropReef',False,'whether to use DropReef')
...
def load_data(data_prefix, dataset_str, precalc,dropreef):
  """Return the required data formats for GCN models."""
  (num_data, train_adj, full_adj, feats, train_feats, test_feats, labels,
   train_data, val_data,
   test_data) = utils.load_graphsage_data(data_prefix, dataset_str, dropreef = dropreef)
...
def main(unused_argv):
  ...
  (train_adj, full_adj, train_feats, test_feats, y_train, y_val, y_test,
   ...
   visible_data) = load_data(FLAGS.data_prefix, FLAGS.dataset, FLAGS.precalc,FLAGS.use_DropReef)
